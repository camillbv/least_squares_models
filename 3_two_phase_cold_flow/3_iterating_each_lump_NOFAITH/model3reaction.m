...
disp('Start med gassfraksjonar, men blei demotivert, då eg er sterkt usikker på om dette er ein god idè.')
pause
...
while  sum(L2_norm_tot) > tol && iter < max_iter % criteria to continue iteration
    ...
    for compNo = 1:nCompGas
        
        %% iteration properties for variable iteration
        iter_variable           = 0; L2_norm_tot_variable    = 1;
        max_iter_variable       = 20; tol_variable            = 1e-8;
        ...
        while L2_norm_tot_variable > tol_variable && iter_variable < max_iter_variable
            ...
            %% calculate L for current gas component
            L1  = D;
            L2 = zeros(N,N);
            L3 = zeros(N,N);
            ...
            for zPoint = 1:N
                L2(zPoint,zPoint) = -1./supVelGas(zPoint).*liqDensity./gasDensity(zPoint).*sum(massTransferTerms(zPoint,:));
                L3(zPoint,zPoint) =  1./supVelGas(zPoint).*liqDensity./gasDensity(zPoint).*kLa(compNo)./equiConst(compNo).*...
                    avMolMassGas(zPoint)./avMolMassLiq(zPoint);
            end
            ...
            g = zeros(N,1);
            for zPoint = 1:N
                g(zPoint) = 1./supVelGas(zPoint).*liqDensity./gasDensity(zPoint).*kLa(compNo).*wtFracLiq(zPoint,compNo)';
            end
            ...
            %% update wtFracGas into matrix again for next iteration
            wtFracGas(:,compNo) = f;
            solVec(:,compNo)    = f;
