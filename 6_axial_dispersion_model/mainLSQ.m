...
while  sum(L2_norm_tot) > tol && iter < max_iter % criteria to continue iteration
    ...
    while L2_norm_tot_variable > tol_variable && iter_variable < max_iter_variable
        ...
        %% L 
        nComp = 7; % for six components
        
        %% make storage for current iteration
        ...
        F_gamma         = zeros(4*N,1);
        F_gamma(1)      = BC(cNo);            % inlet gas weight fraction of CO
        F_gamma(2*N)    = 0;                % outlet gas flux of CO
        F_gamma(2*N+1)  = BC(nCompGas+cNo);   % inlet liquid weight fraction of CO 
