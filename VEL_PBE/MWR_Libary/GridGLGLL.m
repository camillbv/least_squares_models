...
function grid=GridGLGLL(N1, N2, xi_min, xi_max, t_min, t_max)
...
%    grid.dt 
   ...
   xi_=-1:2/40:1;
   grid.Lag.xi=(xi_max-xi_min)/2.*(xi_+1);
   
   grid.Lag.Interp= zeros(length(xi_),N1+1);
    for i=1:N1+1
        grid.Lag.Interp(:,i)=LagrangeGL(i,xi_,N1+1);
    end
...
grid.loc_xi_basis=zeros(grid.N,1);
grid.glb_xi_plane=zeros(grid.N,1);
grid.loc2glb=zeros(grid.Nxi,grid.Nt);

idx=1;
for i=1:grid.Nt
    for j=1:grid.Nxi
        grid.glb_xi_plane(idx)=i;
        grid.loc2glb(j,i)=idx;
        grid.loc_xi_basis(idx)=j;        
...
grid.I=speye(grid.N);
grid.Z=sparse(grid.N,grid.N);

%grid.I=speye(grid.N);
grid.Z_t=sparse(grid.Nt,grid.Nt);
