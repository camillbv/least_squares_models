...
%% see parameter file for to do list

% this code crashes at the estimate of initial values using ODE solver.
...
while  sum(L2_norm_tot) > tol && iter < max_iter % criteria to continue iteration
    ...
    while L2_norm_tot_variable > tol_variable && iter_variable < max_iter_variable
        ...
        reactRate    = get_reactRateSehabiague(parStructReactRate);
    ...
    for zP = 1:N
        massTransSum(zP) = sum(kLa.*liqDensity.*(1./equiConst.* ...
    ...
    % flux equation gas
    L1 = diag(ones(N,1));
    L2 = volFracGas*diag(lambdaGAS)*D;
    L3 = zeros(N,N);
    L4 = zeros(N,N);
    
    % temperature equation gas
    L5 = D;
    L6 = diag(gasDensity)*heatCapGas*diag(supVelGas)*D + ...
         hL*areaDensity*diag(ones(N,1));
    L7 = zeros(N,N);
    L8 = -hL*areaDensity*diag(ones(N,1));
    
    % flux equation liquid
    L9  = zeros(N,N);
    L10 = zeros(N,N);
    L11 = diag(ones(N,1));
    L12 = volFracSlu*diag(lambdaLIQ)*D; % digert!
    
    % temperature equation liquid
    L13 = zeros(N,N);
    L14 = -hL*areaDensity*diag(ones(N,1));
    L15 = D;
    L16 = sluDensityInit*heatCapSluInit*diag(supVelSlu)*D + ...
          hW*perimeter/area*diag(ones(N,1)) + ...
          hL*areaDensity*diag(ones(N,1));
    ...
    %% g
    g = zeros(4*N,1);
    g(3*N+1:4*N) = volFracLiq*(-deltaHr)*reactRate*catDensity + ...
                   (ones(N,1))*tempSurr*hW*perimeter/area;
